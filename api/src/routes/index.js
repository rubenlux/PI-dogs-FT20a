...
//get dogs y get dogs búsqueda por name los hago en la misma ruta porque me trae la misma info (según se ingresa un name o no)
...
router.get("/dogs", async (req, res, next) => {
  try {
    ...
    if (name) {
      ...
      res.status(200).send(dogsTotal); //el otro caso es que no haya un query y manda un status 200 con todos los dogs
    }
  } catch (error) {
    //si hay un error en el try catch lo envio al next.
    next(error);
  }
});
...
router.post("/dog", async (req, res, next) => {
  try {
    ...
    const newDog = await Dog.create({
      //creo el dog con el modelo Dog y le paso lo mismo excepto el temp porque lo tengo que encontrar en un modelo que ya tengo
      name,
      height,
      weight,
      life_span,
